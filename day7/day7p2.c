...
int main() {
    ...
    int min_size = REQUIRED_SPACE - (TOTAL_SPACE - root->size);
    struct directory* current_min = root;

    for (int i = 0; i < num_dirs; i++) {
        if (dirs[i]->size < current_min->size && dirs[i]->size >= min_size) {
            current_min = dirs[i];
        }
    }
    
    printf("We can delete directory %s (%db) in order to free up the required space\n", current_min->name, current_min->size);
    for (int i = 0; i < num_dirs; i++) {
        free(dirs[i]->name);
        free(dirs[i]->sub_dir);
        free(dirs[i]);
