...
int main() {
    ...
    while (fgets(buf, 64, fp)) {
        ...
        int str_cursor = 1;
        switch (mode) {
            // Reading in the initial configuration of the crates
            case 0:
            ...
            case 1:
                ...
                // Transfers crates in order to a different stack
                for (int i = 0; i < input[0]; i++) {
                    ...
                    stacks[input[2]-1][0] = stacks[input[1]-1][input[0]-i-1];
                    
                    for (int j = input[0]-i-1; j < strlen(stacks[input[1]-1]) ; j++) {
